...
class Customer_Query
{
     ...
     * }
     */
    public function __construct($query = '', $db_customer = null)
    {
        if ($db_customer) {
     ...
     * @since  0.0.1
     *
     * @see Customer_Query::__construct()
     *
     * @param string|array $query Array or query string of parameters. See Customer_Query::__construct().
     ...
     *
     * Tries to use a cached value and otherwise uses `Customer_Query::query_items()`.
     *
     * @access protected
     * @since  0.0.1
     *
    ...
    protected function get_items()
    {
         ...
         */
        do_action_ref_array('pre_get_customers', array(&$this));

        // $args can include anything. Only use the args defined in the query_var_defaults to compute the key.
        $key = md5(serialize(wp_array_slice_assoc($this->query_vars, array_keys($this->query_var_defaults))));

        $last_changed = $this->db_customer->get_last_changed();
    ...
    protected function set_found_items()
    {
        ...
        if ($this->query_vars['number'] && !$this->query_vars['no_found_rows']) {
             ...
             */
            $found_items_query = apply_filters('found_customers_query', 'SELECT FOUND_ROWS()', $this);
    ...
    {
        return array_keys($this->db_customer->get_columns());
    }
}
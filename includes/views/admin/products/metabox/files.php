    ...
    <ul class="list-group" id="smartpay_product_files">
        <?php foreach ($files as $index => $file) : ?>
            ...
            <li class="list-group-item list-group-item-action mb-0 files-item" id="file-<?php echo $id ?>">
                <input type="hidden" class="form-control" name="<?php echo 'files[' . $id . '][id]'; ?>" value="<?php echo $id ?>">
                <input type="hidden" class="form-control" name="<?php echo 'files[' . $id . '][icon]'; ?>" value="<?php echo $file['icon'] ?? '' ?>">
                <input type="hidden" class="form-control" name="<?php echo 'files[' . $id . '][filename]'; ?>" value="<?php echo $file['filename'] ?? '' ?>">
                <input type="hidden" class="form-control" name="<?php echo 'files[' . $id . '][mime]'; ?>" value="<?php echo $file['mime'] ?? '' ?>">
                <input type="hidden" class="form-control" name="<?php echo 'files[' . $id . '][size]'; ?>" value="<?php echo $file['size'] ?? '' ?>">
                <input type="hidden" class="form-control" name="<?php echo 'files[' . $id . '][url]'; ?>" value="<?php echo $file['url'] ?? '' ?>">
                <div class="d-flex">
                    <div class="file-type">
